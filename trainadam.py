...
import sys
import importlib
sys.path.insert(0, '/home/duhuipeng/SAMOS')
try:
    import module
    importlib.reload(module)
    from module import RestormerBlock1D
    print("✓ 成功导入 RestormerBlock1D")
except ImportError as e:
    print(f"导入失败: {e}")
    # 如果还失败，直接定义
    print("直接定义 RestormerBlock1D...")
random.seed(1584)
...
class MosPredictor(nn.Module):
    ...
    def forward(self, wav):
        ...
        features = self.restormer_blocks(ssl_feature)
        ...
        mean_pooled = torch.mean(features, dim=1)
...
if __name__ == '__main__':
    ...
    # 设置优化器 - 改为AdamW
    # 根据论文设置: learning_rate=1e-4, betas=(0.9, 0.98), weight_decay=1e-4
    ...
    scheduler = OneCycleLR(optimizer,
                          ...
                          epochs=max_epochs,  # 总epoch数
                          steps_per_epoch=steps_per_epoch,  # 每epoch步数
                          pct_start=0.1,  # 升温阶段占比10%
                          anneal_strategy='cos',  # 余弦退火
                          div_factor=25,  # 初始学习率 = max_lr/25
                          final_div_factor=1e4)  # 最终学习率 = max_lr/(25 * 1e4)
    
    print('Learning rate scheduler: OneCycleLR')
    ...
    with open(results_path, 'w') as f:
        ...
        f.write(f'  - Betas: (0.9, 0.98)\n')
        f.write(f'  - Weight decay: 1e-4\n')
        f.write(f'  - Gradient clipping: {gradient_clip_val}\n')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(ckptdir, 'learning_rate_schedule.png'))
    plt.close()
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(ckptdir, 'training_loss.png'))
    plt.close()
    
    # 绘制验证SRCC曲线
    plt.figure(figsize=(10, 6))
    plt.plot(val_srcc_history, label='Validation SRCC')
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(ckptdir, 'validation_srcc.png'))
    plt.close()
    
    print(f'Training results saved to: {results_path}')
    print(f'Training curves saved to: {ckptdir}')
