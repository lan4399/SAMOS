...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='使用预提取特征训练MOS预测器（验证集最佳->测试集最终）')
    ...
    parser.add_argument('--outdir', type=str, required=False, default='checkpoints', help='输出目录')
    ...
    vallist = os.path.join(args.datadir, 'sets/val_mos_list.txt')  # 验证集
    testlist = os.path.join(args.datadir, 'sets/test_mos_list.txt')  # 测试集（最终评估用）
    ...
    print(f'策略: 每轮在验证集评估SRCC，训练完成后用验证集最佳模型测试测试集')
    ...
    # 创建验证集（每轮评估用）
    valset = PrecomputedFeatureDataset(args.feature_dir, vallist)
    valloader = DataLoader(valset, batch_size=1, shuffle=False, num_workers=2, collate_fn=valset.collate_fn)
    ...
    print('策略: 每轮在验证集评估SRCC，保存验证集最佳模型')
    ...
    for epoch in range(1, args.max_epochs + 1):
        ...
        print('验证集评估中:')
        val_results = evaluate_model(net, valloader, vallist, device, '验证集评估')
        ...
        print(f'[验证集句子级] Spearman等级相关系数= {val_utt_SRCC:.6f}')
        print(f'[验证集句子级] Kendall Tau等级相关系数= {val_metrics["val_utt_KTAU"]:.6f}')
        
        # 检查是否达到最佳验证集SRCC
        if val_utt_SRCC > best_val_utt_SRCC:
            best_val_utt_SRCC = val_utt_SRCC
            best_epoch = epoch
            print(f'\n*** 新的最佳验证集SRCC: {best_val_utt_SRCC:.6f} (第{epoch}轮) ***')
            
            # 保存最佳模型（基于验证集SRCC）
            path = os.path.join(args.outdir, 'best_model_val_srcc.pt')
    ...
    # 训练完成后加载验证集最佳模型进行最终测试集评估
    print(f'\n{"="*60}')
    print('训练完成！加载验证集最佳模型进行最终测试集评估...')
    print(f'最佳轮次: 第{best_epoch}轮')
    print(f'最佳验证集SRCC: {best_val_utt_SRCC:.6f}')
    
    # 加载最佳模型
    best_model_path = os.path.join(args.outdir, 'best_model_val_srcc.pt')
    ...
    print('\n进行最终测试集评估...')
    final_test_results = evaluate_model(net, testloader, testlist, device, '最终测试集评估')
    ...
    plt.tight_layout()
    plot_path = os.path.join(args.outdir, 'val_based_training_progress.png')
    ...
    with open(results_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'最佳验证集SRCC: {best_val_utt_SRCC:.6f}\n')
        ...
        f.write(f'评估策略: 每轮验证集SRCC -> 最终测试集评估\n')
    
    print(f'\n预提取特征MOS预测器训练完成！')
    print(f'最佳模型文件: {best_model_path}')
    print(f'最佳验证集SRCC: {best_val_utt_SRCC:.6f} (第{best_epoch}轮)')
    print(f'最终测试集SRCC: {final_test_utt_metrics["final_test_utt_SRCC"]:.6f}')
